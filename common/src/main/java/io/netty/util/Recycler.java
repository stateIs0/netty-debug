...
public abstract class Recycler<T> {
    ...
    private static final int MAX_DELAYED_QUEUES_PER_THREAD;// cpu * 2
    ...
    public final T get() {
        ...
        // 每个线程都会有一个 Stack 对象，也会有一个 WeakHashMap<Stack<?>, WeakOrderQueue>() 对象
    ...
    static final class Stack<T> {
        ...
        final AtomicInteger availableSharedCapacity;// 2048
        final int maxDelayedQueues;// 8

        private final int maxCapacity;// 4096
        ...
        Stack(Recycler<T> parent, Thread thread, int maxCapacity, int maxSharedCapacityFactor,
            ...
            this.maxDelayedQueues = maxDelayedQueues;//8
        }
        ...
        DefaultHandle<T> pop() {
            int size = this.size;
            // 如果没有可用的了，就从 queue 的 Link 中清理一些过来
            if (size == 0) {
                if (!scavenge()) {
                    return null;
                }
        ...
        boolean dropHandle(DefaultHandle<?> handle) {
            ...
            if (!handle.hasBeenRecycled) {
                ...
                // 如果下面的判断不是0 的话，那么就是这个 ++ 操作出现了并发。这个对象就没有必要重复的放入容器。
                // 直接丢弃。
