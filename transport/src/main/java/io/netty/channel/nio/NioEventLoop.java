...
public final class NioEventLoop extends SingleThreadEventLoop {
  ...
  NioEventLoop(NioEventLoopGroup parent, Executor executor, SelectorProvider selectorProvider,
    ...
    // select 元组，封装了 select 和 selectKey[]
    final SelectorTuple selectorTuple = openSelector();
    selector = selectorTuple.selector;
    unwrappedSelector = selectorTuple.unwrappedSelector;
    // 选择策略 DefaultSelectStrategyFactory
  ...
  private void processSelectedKey(SelectionKey k, AbstractNioChannel ch) {
    ...
    try {
      ...
      if ((readyOps & SelectionKey.OP_WRITE) != 0) {
        ...
        logger.error("OP_WRITE " + readyOps);
  ...
  private void select(boolean oldWakenUp) throws IOException {
    ...
    try {
      ...
      for (; ; ) {
        ...
        if (hasTasks() && wakenUp.compareAndSet(false, true)) {// 含有任务且唤醒 selector 成功
        ...
        selectCnt++;
        // 如果1秒后返回，有返回值 || oldWakenUp 是 true 表示现在是 wakeup 是 false || select 被用户唤醒 || 任务队列有任务 || 有定时任务即将被执行 则跳出循环
        ...
        if (time - TimeUnit.MILLISECONDS.toNanos(timeoutMillis) >= currentTimeNanos) {
          ...
          selectCnt = 1;
        } else if (SELECTOR_AUTO_REBUILD_THRESHOLD > 0 &&// 没有持续 timeoutMillis 且超过 512次
